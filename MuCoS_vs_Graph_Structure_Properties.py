...
# Load the dataset
#file_path = r"D:\Complexities_papers\MuCo_MRR_vs_Features.csv"
file_path = r"D:\dataset_complexity\MuCo_MRR_vs_Features.csv"
...
numerical_columns = [col for col in numerical_columns if col != 'MRR']
...
for i, column in enumerate(numerical_columns, 1):
    ...
    temp_df = df[['Datasets', 'MRR', column]].dropna().sort_values(by='MRR')

    # Check if the column contains enough data to plot
    if temp_df[column].dropna().empty:
        print(f"Warning: No valid data to plot for {column}. Skipping plot.")
        continue  # Skip this iteration and move to the next column
    
    # Calculate dynamic vertical offset based on data range
    y_range = temp_df[column].max() - temp_df[column].min()
    ...
    if np.isnan(y_range) or np.isinf(y_range):
        ...
        print(f"Warning: Invalid y_range detected for {column}. Defaulting to 1.")
    ...
    correlation = temp_df[['MRR', column]].corr().iloc[0, 1]
    ...
    ax.set_xlabel('MRR', fontsize=10, fontweight='bold')  # Make label bold
    ax.set_ylabel(column, fontsize=10, fontweight='bold')  # Set y-axis label to the column name
